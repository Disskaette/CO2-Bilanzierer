...
class ExportDialogPro(ctk.CTkToplevel):
    ...
    def __init__(self, parent, project: Project):
        ...
        self.variant_comments = {}  # Speichert Kommentare als Strings (nicht Widgets!)
    ...
    def _export_pdf(self):
        ...
        try:
            ...
            # Zusatzbild
            config.additional_image_path = self.image_path_var.get() if self.image_path_var.get() else None
            ...
            logger.info(f"Hat dashboard_view: {hasattr(self.parent_window, 'dashboard_view')}")
            logger.info(f"Hat variant_views: {hasattr(self.parent_window, 'variant_views')}")
            ...
            try:
                ...
                if hasattr(self.parent_window, 'dashboard_view'):
                    ...
                    if dashboard_view:
                        logger.info(f"Dashboard-View hat figure: {hasattr(dashboard_view, 'figure')}")
                        if hasattr(dashboard_view, 'figure') and dashboard_view.figure:
                            dashboard_figure = dashboard_view.figure
                            logger.info("✓ Dashboard-Figure aus View extrahiert")
                        else:
                            logger.warning("Dashboard-View hat keine figure oder figure ist None")
                    else:
                ...
                if hasattr(self.parent_window, 'variant_views'):
                    ...
                    logger.info(f"Varianten-Views: {len(variant_views)} vorhanden")
                    logger.info(f"Zu exportierende Varianten-Indizes: {config.include_variants}")
                    
                    for idx in config.include_variants:
                        logger.info(f"--- Versuche Variante {idx} zu extrahieren ---")
                        if idx < len(variant_views):
                            ...
                            if view is None and hasattr(self.parent_window, '_create_variant_view'):
                                logger.info(f"View {idx} ist None, erstelle es für Export...")
                                try:
                                    ...
                                    temp_view = VariantView(None, orchestrator, idx)
                                    
                                    # Hole Figure aus der temporären View
                                    if hasattr(temp_view, 'figure') and temp_view.figure:
                                        variant_figures[idx] = temp_view.figure
                                        logger.info(f"✓ Varianten-Figure {idx} ({self.project.variants[idx].name}) temporär erstellt")
                                    else:
                                        logger.warning(f"Temporäre View {idx} hat keine figure")
                                    
                                except Exception as e:
                                    logger.error(f"Fehler beim Erstellen der temporären View {idx}: {e}", exc_info=True)
                            
                            elif view:
                                logger.info(f"View {idx} hat figure: {hasattr(view, 'figure')}")
                                if hasattr(view, 'figure') and view.figure:
                                    variant_figures[idx] = view.figure
                                    logger.info(f"✓ Varianten-Figure {idx} ({self.project.variants[idx].name}) aus View extrahiert")
                                else:
                                    logger.warning(f"View {idx} hat keine figure oder figure ist None")
                            else:
                                logger.warning(f"View {idx} ist None und konnte nicht erstellt werden")
                        else:
                            logger.warning(f"Index {idx} außerhalb des View-Arrays ({len(variant_views)})")
                
                logger.info(f"=== Extraktion abgeschlossen: Dashboard={dashboard_figure is not None}, Varianten={len(variant_figures)} ===")
                
            except Exception as e:
                logger.error(f"Fehler beim Extrahieren der Figuren: {e}", exc_info=True)
            
            # Export mit bestehenden Figuren
