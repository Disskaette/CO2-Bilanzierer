...
class CustomMaterialDialog(ctk.CTkToplevel):
    ...
    def _build_ui(self) -> None:
        ...
        # Name (Pflicht)
        name_frame = self._create_field_frame(
            main_frame, "Name *", is_required=True)
        self.name_entry = ctk.CTkEntry(
            name_frame, width=400, placeholder_text="z.B. Holzfaserplatte 160 kg/m³")
        self.name_entry.pack(fill="x", padx=5, pady=5)

        # Quelle/Hersteller
        source_frame = self._create_field_frame(
            main_frame, "Quelle/Hersteller")
        self.source_entry = ctk.CTkEntry(
            source_frame, width=400, placeholder_text="z.B. Hersteller XY AG")
        ...
        self.type_combo = ctk.CTkComboBox(
            type_frame,
            values=["generisch", "spezifisch",
                    "durchschnitt", "repräsentativ"],
            width=400
        )
        ...
        # Konformität/Norm
        conformity_frame = self._create_field_frame(
            main_frame, "Norm/Konformität")
        ...
        # Einheit
        unit_frame = self._create_field_frame(
            main_frame, "Einheit *", is_required=True)
        self.unit_combo = ctk.CTkComboBox(
            unit_frame,
            values=["kg", "t", "m²", "m³", "m", "Stück"],
            width=400
        )
        ...
        # GWP A1-A3 (Pflicht)
        a1a3_frame = self._create_field_frame(
            main_frame, "GWP A1-A3 (Herstellung) *", is_required=True)
        self.gwp_a1a3_entry = ctk.CTkEntry(
            a1a3_frame, width=400, placeholder_text="z.B. 1.234")
        self.gwp_a1a3_entry.pack(fill="x", padx=5, pady=5)

        # GWP C3
        c3_frame = self._create_field_frame(
            main_frame, "GWP C3 (Abfallbehandlung)")
        self.gwp_c3_entry = ctk.CTkEntry(
            c3_frame, width=400, placeholder_text="Optional, z.B. 0.123")
        self.gwp_c3_entry.pack(fill="x", padx=5, pady=5)

        # GWP C4
        c4_frame = self._create_field_frame(
            main_frame, "GWP C4 (Deponie/Recycling)")
        self.gwp_c4_entry = ctk.CTkEntry(
            c4_frame, width=400, placeholder_text="Optional, z.B. -0.050")
        self.gwp_c4_entry.pack(fill="x", padx=5, pady=5)

        # GWP D
        d_frame = self._create_field_frame(main_frame, "GWP D (Gutschriften)")
        self.gwp_d_entry = ctk.CTkEntry(
            d_frame, width=400, placeholder_text="Optional, z.B. -0.200")
        ...
        bio_frame = self._create_field_frame(
            main_frame, "Biogene CO₂-Speicherung")
        ...
        self.bio_carbon_entry = ctk.CTkEntry(
            bio_frame, width=400, placeholder_text="Optional, z.B. -1.830")
    ...
    def _validate_float(self, value_str: str, field_name: str, allow_empty: bool = False) -> Optional[float]:
        ...
        if not value_str:
            if allow_empty:
                ...
                raise ValueError(
                    f"{field_name}: Pflichtfeld darf nicht leer sein")
        ...
        try:
            ...
            raise ValueError(
                f"{field_name}: Ungültiger Zahlenwert '{value_str}'")
    ...
    def _on_save(self) -> None:
        ...
        try:
            ...
            gwp_a1a3 = self._validate_float(
                self.gwp_a1a3_entry.get(), "GWP A1-A3", allow_empty=False)
            gwp_c3 = self._validate_float(
                self.gwp_c3_entry.get(), "GWP C3", allow_empty=True) or 0.0
            gwp_c4 = self._validate_float(
                self.gwp_c4_entry.get(), "GWP C4", allow_empty=True) or 0.0
            gwp_d = self._validate_float(
                self.gwp_d_entry.get(), "GWP D", allow_empty=True)

            # Biogener Kohlenstoff
            biogenic_carbon = self._validate_float(
                self.bio_carbon_entry.get(), "Biogener Kohlenstoff", allow_empty=True)
